...
export async function createShareLink(
  ...
  const response = await fetch(
    buildPath(
      type === "recv"
        ? `/recv._ajax.php?action=shareMail&nopost=1&&num=${id}&csrf_token_check=${csrfToken}&csrf_subtoken_check=${csrfSubToken}&_=${Date.now()}`
        : `/send._ajax.php?action=shareMail&nopost=1&&num=${id}&_=${Date.now()}`,
    ),
